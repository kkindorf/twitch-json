...
var panels = $(".panels");
$(function(){
  for (var i = 0; i < twitchJSON.length; i++){
    if(twitchJSON[i].display_name != undefined){
      var name = twitchJSON[i].display_name;
      var logo = twitchJSON[i].logo;
      var url = twitchJSON[i].url;
      var status = twitchJSON[i].status;
      panels.append("<div class='panel'><div class='img-wrapper'><img class='logo' src="+logo+"></div><div class='title'><h2 class='name'>"+name+"</h2></div><a href="+url+" target='_blank'><p class='status'>Status: "+status+"</p></a></div>");
    }
  }
})
